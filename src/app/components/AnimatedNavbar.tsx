        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between rounded-full border border-[#AFD5BC]/30 bg-[#1E3159]/90 px-2.5 py-1.5 shadow-[0_12px_40px_rgba(15,24,45,0.26)] backdrop-blur-xl sm:px-4 sm:py-2 lg:px-5">
          <Link href="#home" className="flex min-w-0 items-center gap-1.5 sm:gap-3" onClick={closeMobileMenu}>
            ...
            <div className="flex min-w-0 flex-col leading-none">
              ...
              <span className="mt-1 hidden text-[0.58rem] font-semibold uppercase tracking-[0.28em] text-white/84 min-[420px]:block">
                2026-2027
              </span>
            </div>
          </Link>
                  ...
                  <Link
                    ...
                    className={`rounded-full px-3.5 py-2 text-sm font-semibold uppercase tracking-[0.24em] transition-all duration-300 ${
                      isPrimary
                        ...
                        : "text-white/90 hover:bg-[#AFD5BC]/15 hover:text-[#AFD5BC]"
                    }`}
                  >
                    {item.label}
                  </Link>
              ...
              <nav className="mt-6 flex flex-col gap-2">
                {navItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="rounded-full border border-transparent px-4 py-3 text-left text-base font-semibold uppercase tracking-[0.24em] text-white/92 transition-all duration-300 hover:border-[#AFD5BC]/30 hover:bg-[#AFD5BC]/10 hover:text-[#AFD5BC]"
                    onClick={closeMobileMenu}
                  >
                    {item.label}
                  </Link>
